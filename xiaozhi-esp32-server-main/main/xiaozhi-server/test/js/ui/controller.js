...
class UIController {
    ...
    initEventListeners() {
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    this.addChatMessage('⚠️ Camera start failed, please check browser permissions', false);
                                }
                            }).catch(error => {
                                log(`Exception starting camera: ${error.message}`, 'error');
                            });
                        } else {
                            log('startCamera function not defined', 'warning');
                        }
                    }
                }, 300);
            });
        ...
        // Click modal background to close (only disabled for specific modals)
            ...
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    ...
                    if (nonClosableModals.includes(modal.id)) {
                        return; // Disable clicking background to close
                    }
                    this.hideModal(modal.id);
                }
            });
    ...
    updateDialButton(isConnected) {
        ...
        if (dialBtn) {
            if (isConnected) {
                ...
                dialBtn.querySelector('.btn-text').textContent = 'Hangup';
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Current access is via HTTP, recording is unavailable, text interaction only' : 'Microphone unavailable';
                } else {
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = 'Record';
            recordBtn.title = isHttpNonLocalhost ? 'Current access is via HTTP, recording is unavailable, text interaction only' : 'Microphone unavailable';
            ...
            app.live2dManager.switchModel(selectedModel)
                .then(success => {
                    if (success) {
                        ...
                        this.addChatMessage('Model switch failed', false);
                    }
                })
                .catch(error => {
                    console.error('Model switch error:', error);
                    this.addChatMessage('Model switch error', false);
                });
        } else {
    ...
    startAIChatSession() {
        ...
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Current access is via HTTP, recording is unavailable, text interaction only', false);
            } else {
                this.addChatMessage('⚠️ Microphone unavailable, please check permission settings, text interaction only', false);
            }
        }
        ...
        if (window.cameraAvailable && typeof window.startCamera === 'function') {
            window.startCamera().then(success => {
                if (success) {
                    ...
                    this.addChatMessage('⚠️ Camera start failed, possibly rejected by browser', false);
                }
            }).catch(error => {
                log(`Exception starting camera: ${error.message}`, 'error');
            });
    ...
    async handleConnect() {
        ...
        if (!otaUrlInput || !otaUrlInput.value) {
            this.addChatMessage('Please enter OTA server address', false);
            return;
        }
