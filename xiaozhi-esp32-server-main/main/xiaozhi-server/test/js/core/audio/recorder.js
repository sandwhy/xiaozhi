...
export class AudioRecorder {
    ...
    async createAudioProcessor() {
        ...
        try {
            if (this.audioContext.audioWorklet) {
                ...
                log('AudioWorklet unavailable, using ScriptProcessorNode as fallback', 'warning');
    ...
    createScriptProcessor() {
        try {
            ...
            silent.connect(this.audioContext.destination);
            log('Successfully used ScriptProcessorNode as fallback', 'warning');
    ...
    async start() {
        ...
        try {
            ...
            log('Please record at least 1-2 seconds of audio to ensure enough data is collected', 'info');
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                log(`Recording start message sent`, 'info');
            } else {
            ...
            log('Started direct PCM recording', 'success');
            return true;
        } catch (error) {
    ...
    stop() {
        ...
        try {
            ...
            log('Stopped direct PCM recording', 'success');
            return true;
        } catch (error) {
