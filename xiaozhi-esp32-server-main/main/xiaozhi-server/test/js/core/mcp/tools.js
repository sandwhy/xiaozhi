            ...
            <div class="mcp-tool-card">
                ...
                <div class="mcp-tool-info">
                    <div class="mcp-tool-info-row">
                        ...
                        <span class="mcp-tool-info-value">${paramCount} item(s) ${requiredCount > 0 ? `(${requiredCount} required)` : ''}</span>
                    </div>
                    <div class="mcp-tool-info-row">
...
/**
 * Update visibility of numerical range input boxes
 */
...
function handlePropertySubmit(e) {
    ...
    // Add range limits for numerical types
    if (type === 'integer' || type === 'number') {
        if (minimum !== '') {
...
function setupMcpEventListeners() {
    ...
    // Parameter edit modal related elements
...
function handleMcpSubmit(e) {
    ...
    // Parse mock return results
    let mockResponse = null;
    if (mockResponseText) {
        try {
            mockResponse = JSON.parse(mockResponseText);
        } catch (e) {
            showMcpError('Mock response is not valid JSON format: ' + e.message);
            return;
        }
    }
    // Build inputSchema
...
export async function executeMcpTool(toolName, toolArgs) {
    ...
    // Handle photo taking tool
    if (toolName === 'self_camera_take_photo') {
        if (typeof window.takePhoto === 'function') {
            const question = toolArgs && toolArgs.question ? toolArgs.question : 'Describe the item seen';
            log(`Taking photo: ${question}`, 'info');
            const result = await window.takePhoto(question);
            return result;
        } else {
            log('Photo taking function unavailable', 'warning');
            return { success: false, error: 'Camera not started or does not support photo taking' };
        }
    }

    // If there is a mock return result, use it
    if (tool.mockResponse) {
        // Replace template variables
        let responseStr = JSON.stringify(tool.mockResponse);
        // Replace variables in ${paramName} format
        if (toolArgs) {
    ...
    // No mock return result, return default success message
