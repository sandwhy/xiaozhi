...
class OTAHandler(BaseHandler):
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If non-empty string is configured
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # If signature fails, leave empty, device decides if passwordless is allowed
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signature key, password left empty")

                # Build MQTT configuration (directly use mqtt_gateway string)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Issued MQTT gateway configuration for device {device_id}")

            else:  # mqtt_gateway not configured, issue WebSocket
                # If authentication is enabled, perform authentication check
                token = ""
                if self.auth_enable:
                ...
                self.logger.bind(tag=TAG).info(
                    f"MQTT gateway not configured, issuing WebSocket configuration for device {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Searching for firmware for model {device_model}, found {len(candidates)} candidates"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Issued firmware {chosen_version} for device {device_id} [if address prefix is incorrect, please check server.vision_explain in config file] -> {chosen_url}"
                    )
                else:
                    self.logger.bind(tag=TAG).info(
    ...
    async def handle_download(self, request):
        ...
        - Only .bin files in the data/bin directory are allowed to be downloaded
        - filename must be a basename and match a secure pattern
        """
        try:
