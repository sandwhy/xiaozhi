...
def initialize_modules(
    ...
    if init_asr:
        ...
        logger.bind(tag=TAG).info(f"Component initialized: asr success {select_asr_module}")
    return modules
...
def initialize_voiceprint(asr_instance, config):
    ...
    if not voiceprint_config.get("url") or not voiceprint_config.get("speakers"):
        logger.bind(tag=TAG).warning("Voiceprint configuration incomplete")
        return False
    ...
    try:
        ...
        logger.bind(tag=TAG).error(f"Dynamic voiceprint initialization failed: {str(e)}")
        return False
