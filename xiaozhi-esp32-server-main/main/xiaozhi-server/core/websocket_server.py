    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            ...
            if "device-id" not in query_params:
                await websocket.send("Port is normal, if you need to test the connection, please use test_page.html")
                await websocket.close()
                return
            else:
        ...
        """Process new connection, create an independent ConnectionHandler each time"""
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Error processing connection: {e}")
        finally:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(
                    f"Error forcing connection close on server side: {close_error}"
                )
    ...
    async def update_config(self) -> bool:
        ...
        try:
            async with self.config_lock:
                # Re-fetch configuration (using async version)
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If it is a whitelisted device, do not check token, let it pass
                return
            else:
