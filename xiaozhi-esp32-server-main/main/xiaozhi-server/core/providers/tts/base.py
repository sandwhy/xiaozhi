...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        # Mapping from sentence_id to text, used for getting correct subtitle text in streaming TTS
    ...
    def to_tts_stream(self, text, opus_handler: Callable[[bytes], None] = None) -> None:
        ...
        if self.delete_audio_file:
            # If file deletion is needed, convert directly to audio data
            ...
            try:
                while not os.path.exists(tmp_file) and max_repeat_time > 0:
                    try:
                        ...
                        # Failure, delete file if it exists
    ...
    async def open_audio_channels(self, conn):
        ...
        # Create encoder based on conn sample_rate; do not overwrite if subclass already created it (IndexTTS returns 24kHZ - needs resampling)
        ...
        # TTS consumption thread
        self.tts_priority_thread = threading.Thread(
            target=self.tts_text_priority_thread, daemon=True
        )
        self.tts_priority_thread.start()

        # Audio playback consumption thread
    ...
    def store_tts_text(self, sentence_id, text):
        """Store text for specified sentence_id, used for getting correct subtitle text in streaming TTS
        ...
        if sentence_id and text:
            ...
            # Keep only the most recent 5 to prevent memory leaks
    ...
    # Please override for streaming processing in subclasses
    ...
    def _audio_play_priority_thread(self):
        # List of text and audio to be reported
        enqueue_text = None
        enqueue_audio = []
        while not self.conn.stop_event.is_set():
            text = None
            try:
                ...
                if self.conn.client_abort:
                    logger.bind(tag=TAG).debug("Received interruption signal, skipping current audio data")
                ...
                if sentence_type is not SentenceType.MIDDLE:
                    if self.report_on_last:
                        # Accumulative mode: suitable for TTS with only one voice stream (e.g., seed-tts-2.0)
                        # Record text on FIRST, accumulate audio, report all on LAST
                        if text:
                            enqueue_text = text
                        ...
                        # Non-accumulative mode: report each sentence separately
                        if enqueue_text is not None:
                            enqueue_tts_report(self.conn, enqueue_text, enqueue_audio)
                        enqueue_audio = []
                        enqueue_text = text

                # Collect reporting audio data
    ...
    def _get_segment_text(self):
        ...
        # Select different punctuation set based on whether it's the first sentence
        punctuations_to_use = (
            self.first_sentence_punctuations
        ...
        if last_punct_pos != -1:
            ...
            # If first sentence, set flag to False after finding first comma
            if self.is_first_sentence:
    ...
    def _process_remaining_text_stream(
        ...
        Returns:
            bool: Successfully processed text
        """
        full_text = "".join(self.tts_text_buff)
    ...
    def _apply_percentage_params(self, config):
        """Apply percentage parameters in bulk based on TTS_PARAM_CONFIG defined in subclass"""
