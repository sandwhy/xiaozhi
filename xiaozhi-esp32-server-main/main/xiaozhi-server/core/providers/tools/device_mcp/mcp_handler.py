...
class MCPClient:
    """Device-side MCP client, used for managing MCP status and tools"""
...
async def send_mcp_message(conn: "ConnectionHandler", payload: dict):
    ...
    if not conn.features.get("mcp"):
        logger.bind(tag=TAG).warning("Client does not support MCP, unable to send MCP message")
        return
...
async def handle_mcp_message(
    ...
    """Process MCP message, including initialization, tool list, and tool call responses"""
    logger.bind(tag=TAG).debug(f"Process MCP message: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message missing payload field or format incorrect")
        return
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            logger.bind(tag=TAG).debug("Initialization complete, starting to request MCP tool list")
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                if next_cursor:
                    ...
                    logger.bind(tag=TAG).debug("All tools obtained, MCP client ready")
...
async def call_mcp_tool(
    ...
    try:
        if isinstance(args, str):
                    ...
                    try:
                        ...
                        if len(json_objects) > 1:
                            ...
                            if merged_dict:
                                ...
                                raise ValueError(f"Unable to parse any valid JSON objects: {args}")
                        else:
            ...
            raise ValueError(f"Incorrect parameter type, expected string or dictionary, actual type: {type(args)}")
