...
class ASRProviderBase(ABC):
        ...
        while not conn.stop_event.is_set():
            try:
                ...
                logger.bind(tag=TAG).error(
                    f"Failed to process ASR text: {str(e)}, Type: {type(e).__name__}, Traceback: {traceback.format_exc()}"
                )
                continue
    ...
    async def receive_audio(self, conn: "ConnectionHandler", audio, audio_have_voice):
        if conn.client_listen_mode == "manual":
            # Manual mode: buffer audio for ASR recognition
            conn.asr_audio.append(audio)
        else:
            # Auto/Real-time mode: use VAD detection
            conn.asr_audio.append(audio)

            # If no voice and no previous voice, buffer some audio
            if not audio_have_voice and not conn.client_have_voice:
                conn.asr_audio = conn.asr_audio[-10:]
                return

            # Trigger recognition when voice stop is detected via VAD in auto mode
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            # If in exit process, close connection directly, do not process new messages
            if conn.close_after_chat or conn.is_exiting:
                logger.bind(tag=TAG).info("New message received during exit process, closing connection directly")
                await conn.close()
                return
            ...
            logger.bind(tag=TAG).error(f"Failed to process voice stop: {e}")
            import traceback
    ...
    def prefers_temp_file(self) -> bool:
        """Whether to prioritize temporary files"""
        return False
    ...
    def build_temp_file(self, pcm_bytes: bytes) -> Optional[str]:
        try:
            ...
            logger.bind(tag=TAG).error(f"Failed to generate temporary audio file: {e}")
            return None
    ...
    async def speech_to_text(
        ...
        :param audio_format: Audio format, default "opus"
        :param artifacts: Audio artifacts containing PCM data, file path, etc.
    ...
    def decode_opus(opus_data: List[bytes]) -> List[bytes]:
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"Error occurred during audio decoding: {e}")
            return []
        finally:
